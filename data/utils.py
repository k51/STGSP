...
def timestamp2array(timestamps, timeenc, halfhour_flag=False):
    vec_mday = [time.strptime(str(t)[:10], '%Y-%m-%d').tm_mday for t in timestamps]
    ...
    for idx, wday in enumerate(vec_wday):
        if timeenc == 'm':
            # day of month
            v_mday = [0 for _ in range(31)]
            v_mday[vec_mday[idx]-1] = 1
